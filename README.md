...
- Use JSON API errors and basic rate limiting for login and registration.
...
- JSON-ошибки API и базовое ограничение частоты для входа и регистрации.
...
- New passwords are stored as Argon2id hashes with a unique salt.
- Old PBKDF2 password hashes are still accepted and are upgraded after successful login.
...
- Новые пароли хранятся как Argon2id-хэши с уникальной солью.
- Старые PBKDF2-хэши паролей принимаются и обновляются после успешного входа.
- Сессии входа работают через httpOnly cookie.
...
- backend/cmd/api: backend entry point, auth, goals, sessions, stats, security helpers, API helpers, and tests
...
Not implemented yet: password reset by email, email change, account deletion, deployment setup, and final statistics polish.
...
Пока не реализовано: сброс пароля через email, смена email, удаление аккаунта, настройка деплоя и финальная доработка статистики.
