...
Production mode requires HTTPS, secure cookies, an explicit public origin, real SMTP credentials with STARTTLS, VAPID keys for Web Push, persistent SQLite storage, and off-host backups. The production Compose stack creates and verifies daily SQLite backups and keeps them in the `progress-backups` volume according to the configured retention period.

Secrets are supplied only through deployment environment variables. Local environment files, SQLite databases, TLS private keys, and certificate bundles are excluded from Git.

## Эксплуатация

Production-режим требует HTTPS, защищённых cookie, явного публичного origin, реального SMTP с STARTTLS, VAPID-ключей для Web Push, постоянного хранилища SQLite и внешних резервных копий. Production Compose ежедневно создаёт и проверяет SQLite-backup, сохраняя копии в томе `progress-backups` в пределах настроенного срока хранения.

Секреты передаются только через переменные окружения при развёртывании. Локальные файлы окружения, базы SQLite, приватные TLS-ключи и сертификаты исключены из Git.
